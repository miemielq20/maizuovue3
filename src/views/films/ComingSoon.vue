<template>
    <div class="list"> 
      <van-list v-model="obj.loading" :finished="obj.finished" finished-text="没有更多了" @load="onLoad" :immediate-check="false">
        <van-cell v-for="data in obj.datalist" :key="data.filmId" @click="handChangePage(data.filmId)" class="list-item">
          <img :src="data.poster">
          <div class="nowPlayingFilm-info">
            ...
            <div>上映日期：  <span>{{ getDate(data.premiereAt) }}</span></div>
          </div>
        </van-cell>
      </van-list>
    </div>
  ...
  onMounted(() => {
    toast1()
    http({
        url: `/gateway?cityId=${store.state.cityId}&pageNum=1&pageSize=10&type=2&k=2321005`,
        headers: {
    ...
    //转化时间戳为具体日期
    const getDate=(date)=>{
        return moment(date * 1000).format('ddd MoDo')
    }
    ...
    const actorFilter=(data)=>{
     ...
     const onLoad=()=>{
        ...
        //请求列表数据
        http({
        url: `/gateway?cityId=${store.state.cityId}&pageNum=${obj.current}&pageSize=10&type=2&k=2321005`,
        headers: {
          'X-Host': 'mall.film-ticket.film.list'
        }
      }).then(res => {
